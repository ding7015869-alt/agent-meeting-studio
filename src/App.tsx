...
const uiText = {
  zh: {
    ...
    configCopyText: "你现在正在和我对话 —— 所以你就是我要接入的 AI。请把你自己配进 agents.config.json。\\n\\n优先级从高到低：\\n1. 如果你有 API 端点（DeepSeek / OpenAI / 等），用 openai-compatible 模式，填你的 baseUrl、model、apiKeyEnv（引用环境变量名，不要写明文 key）。3 个 agent 共用同一套 API。\\n2. 没有 API 就用 command 模式，command 填你自己的 CLI 启动命令，args 用 {{prompt}}。\\n\\n3 个角色：主理人（拆题拍板）、思路位（发散方案）、评估位（判断利弊）。共用一个后端，但 stance / mission 不同，color 区分。\\n\\n不要建议本地模型（Ollama / vLLM）。如果 API 和 CLI 都不可用，停下来告诉我，让我决定下一步。",
  ...
  en: {
    ...
    configCopyText: "You are the AI I'm talking to right now — so you are the backend to configure. Please plug yourself into agents.config.json.\\\\n\\\\nPriority order (highest first):\\\\n1. If you have an API endpoint (DeepSeek / OpenAI / etc.), use openai-compatible mode with your baseUrl, model, and apiKeyEnv (env var name only, never plaintext key). All 3 agents share one API backend.\\\\n2. No API? Use command mode with your own CLI launch command, args with {{prompt}}.\\\\n\\\\n3 roles: Lead (decompose + decide), Ideator (diverse approaches), Evaluator (pros/cons + risks). Same backend, different stance/mission, distinct colors.\\\\n\\\\nDo NOT suggest local models (Ollama / vLLM). If neither API nor CLI is available, stop and ask me what to do next.",
