...
const uiText = {
  zh: {
    ...
    configCopyPrompt: "复制这段话发给你的 AI，它可以帮你快速配好 Agent Meeting Studio 的参会人员：",
    configCopyText: "请帮我配置 Agent Meeting Studio 的 agents.config.json。我需要 3 个角色：\n1. 主理人 — 负责拆解议题、抛出事项、最终拍板和汇总方案\n2. 思路位 — 负责发散多种方案，从不同角度打开思路\n3. 评估位 — 负责判断利弊、风险和可行性，给出决策建议\n\n每个角色需要填写：id、name（显示名）、title（一句话定位）、stance（核心立场）、style（说话风格）、mission（任务边界）。\n\n接入方式二选一：\n- command 模式：填写 command 和 args 字段，指向本地 CLI（如 hermes / codex），args 中 {{prompt}} 作为站位符\n- openai-compatible 模式：填写 baseUrl、model，配合 Ollama / vLLM / LM Studio\n\n参考示例在 agents.config.example.json 里。请根据你使用的 AI 工具和模型，生成一份可用的配置。",
  ...
  en: {
    ...
    configCopyPrompt: "Copy this and send it to your AI — it can quickly set up your Agent Meeting Studio participants:",
    configCopyText: "Please help me configure agents.config.json for Agent Meeting Studio. I need 3 roles:\n1. Lead — decomposes topics, throws items, makes final decisions, and compiles the plan\n2. Ideator — generates diverse approaches from different angles\n3. Evaluator — judges pros/cons, risks, and feasibility, gives decision recommendations\n\nEach role needs: id, name (display), title (one-liner), stance (core position), style (tone), mission (scope).\n\nConnection mode (pick one):\n- command: set command and args fields pointing to a local CLI (e.g. hermes / codex), use {{prompt}} as placeholder in args\n- openai-compatible: set baseUrl and model for Ollama / vLLM / LM Studio\n\nSee agents.config.example.json for reference. Generate a working config based on the AI tools and models you use.",
